...
class Hand(object):
    ...
    LCS = {'H':'h', 'D':'d', 'C':'c', 'S':'s'}
    SYMBOL = {'USD': '$', 'CAD': 'C$', 'EUR': u'€', 'GBP': u'£', 'SEK': 'kr.', 'RSD': u'РСД', 'mBTC': u'ⓑ', 'INR': u'₹', 'CNY': u'¥', 'T$': '', 'play': ''}
    ...
    def addHoleCards(self, street, player, open=[], closed=[], shown=False, mucked=False, dealt=False):
        ...
        try:
            ...
            log.error(("Hand.addHoleCards: '%s': Major failure while adding holecards: '%s'"), self.handid, e)
    ...
    def getHandId(self, db, id):
        if db.isDuplicate(self.siteId, self.hands['siteHandNo'], self.hands['heroSeat'], self.publicDB):
            #log.debug(("Hand.insert(): hid #: %s is a duplicate") % self.hands['siteHandNo'])
    ...
    def updateSessionsCache(self, db, tz, doinsert = False):
        ...
        if self.cacheSessions:
            heroes = [list(self.dbid_pids.values())[0]]
    ...
    def select(self, db, handId):
        ...
        res = [dict(line) for line in [list(zip([ column[0].lower() for column in c.description], row)) for row in c.fetchall()]]
        for row in res:
            ...
            if game[0] == 'hold' and cardlist[0] != '':
                ...
                streets = dict((v, k) for (k, v) in list(game[3].items()))
        ...
        if res['runittwice'] or self.gametype['split']:
            ...
            q = q.replace('%s', db.sql.query['placeholder'])
            c.execute(q, (handId,))
            boards = [dict(line) for line in [list(zip([ column[0].lower() for column in c.description], row)) for row in c.fetchall()]]
            for b in boards:
                cards = list(map(Card.valueSuitFromCard, [b['boardcard1'], b['boardcard2'], b['boardcard3'], b['boardcard4'], b['boardcard5']]))
                if cards[0]:
        ...
        for row in res:
            ...
            log.debug("Hand.select():: name: '%s' street: '%s' act: '%s' bet: '%s'",
                      name, street, act, bet)
            if   act == 1: # Ante
    ...
    def checkPlayerExists(self,player,source = None):
        ...
        if player not in (p[1] for p in self.players):
            if source is not None:
                log.error(("Hand.%s: '%s' unknown player: '%s'"), source, self.handid, player)
            raise FpdbParseError
        else:
    ...
    def __init__(self, config, hhc, sitename, gametype, handText, builtFrom = "HHC", handid=None):
        ...
        if builtFrom == "HHC":
            ...
            if self.streets['DEAL'] is None:
                log.error("DrawHand.__init__: " + ("Street 'DEAL' is empty. Was hand '%s' cancelled?") % self.handid)
                raise FpdbParseError
            hhc.readBlinds(self)
            hhc.readSTP(self)
            hhc.readAntes(self)
...
class Pot(object):
    ...
    def end(self):
        ...
        # Handle antes
        ante_pots = []
        min_ante = min(self.antes.values()) if self.antes else Decimal(0)
        max_ante = max(self.antes.values()) if self.antes else Decimal(0)
        if max_ante > 0:
            main_ante_pot = sum(min(ante, min_ante, self.common.get(player, Decimal(0))) for player, ante in self.antes.items())
            ante_pots.append((main_ante_pot, set(self.antes.keys())))
            
            # Create a pot for ante difference if necessary
            ante_diff = sum(max(Decimal(0), min(self.antes[player], self.common.get(player, Decimal(0))) - min_ante) for player in self.antes)
            if ante_diff > 0:
                ante_pots.append((ante_diff, {player for player, ante in self.antes.items() if ante > min_ante and self.common.get(player, Decimal(0)) >= ante}))
        
        print(f"Ante pots: {ante_pots}")

        # Calculate main pots (without antes)
        self.total = sum(self.committed.values()) + sum(pot[0] for pot in ante_pots) + self.stp
        # Include all players with committed bets
        commitsall = sorted([(v, k) for (k, v) in self.committed.items() if v > 0])

        main_pots = []
        while commitsall:
            v1 = commitsall[0][0]
            involved_players = set(k for (v, k) in commitsall)
            pot_amount = sum(min(v, v1) for (v, k) in commitsall)
            main_pots.append((pot_amount, involved_players))
            commitsall = [(v - v1, k) for (v, k) in commitsall if v - v1 > 0]

        # Combine ante pots with main pots
        self.pots = ante_pots + main_pots

        # Handle returned bets (uncalled portion)
        self.returned = {}
        for player in self.contenders:
            committed = self.committed[player]
            # Maximum bet among other active players
            max_other_committed = max([self.committed[p] for p in self.contenders if p != player], default=Decimal(0))
            uncalled = committed - max_other_committed
            if uncalled > 0:
                self.returned[player] = uncalled
                self.total -= uncalled

        # Recalculate pots after handling returned bets
        self.pots = [pot for pot in self.pots if pot[0] > 0]

        print(f"Total pot: {self.total}")
        print(f"Pots: {self.pots}")
